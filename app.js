...
async function enrollFaceId() {
    try {
        ...
        const credential = await navigator.credentials.create({
            publicKey: {
                challenge: challenge,
                rp: { name: 'NPT Consultant And Service' },
                user: {
                    id: userId,
                    name: state.currentUser.email,
                    displayName: localStorage.getItem(REMEMBER_NAME_KEY) || state.currentUser.email
                },
        ...
        localStorage.setItem(WEBAUTHN_CRED_KEY, credIdB64);
...
window.resetStaffPin = async function(email, name) {
    ...
    try {
        const response = await fetch(`${API_BASE}/reset-pin`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ targetEmail: email, actingEmail: state.currentUser.email })
        });
        const result = await response.json();
        if (result.success) {
